    ...
    {bookmarks.map(bmark => (
      ...
      <div onMouseEnter={() => {
        ...
        <Collapse accordion activeKey={activeKey} className="card" bordered={false}>
          <Panel style={{ border: '0px'}} showArrow={false} header={<div className="card-header">
            <a className="bookmark-link" href={bmark.link} target="_blank">{bmark.title}</a>
            <BookmarkMenu /></div>} key={bmark.id}>
