    ...
    {bookmarks.filter(bmark => bmark.title.toLowerCase().includes(urlParam.toLowerCase())).map(bmark => (
      ...
      <div onMouseEnter={() => {
        ...
        <Collapse accordion activeKey={activeKey} className="card" bordered={false}>
          <Panel style={{ border: '0px'}} showArrow={false} header={<a className="bookmark-link" href={bmark.link} target="_blank">{<div className="card-header">{bmark.title}<BookmarkMenu title={bmark.title} description={bmark.description} link={bmark.link} id={bmark.id} /></div>}</a>} key={bmark.id}>
