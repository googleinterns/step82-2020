...
def store_user():
    ...
    email_result = list(datastore_client.query(kind='user').add_filter('email', '=', email).fetch(limit=1))[0]
    username_result = list(datastore_client.query(kind='user').add_filter('username', '=', username).fetch(limit=1))[0]
...
def add_clink():
    title = list(datastore_client.query(kind='clink').add_filter('title', '=', request.json['title']).fetch(limit=1))
